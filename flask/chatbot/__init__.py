...
def get_openai_key():
    ...
    try:
        ...
        key_path = "C:\\Users\\v\\Desktop\\codiya\\openai_key.json"
...
def moderate_prompt(prompt: str) -> str:
    ...
    negative_prompt = "\n".join([negative_prefix + negative_suffix])

    return ", ".join([prompt_prefix, prompt.replace('.', ''), prompt_suffix, ]).strip() + "\n" + negative_prompt
...
def generate_image_sync(prompt):
    ...
    save_path = 'flask/static/images/created_image/'+title+'.png'
    print(img)
...
# def main(text, age, gender):
    
#     fixed_gpt_system_prompt = f"""You're a chatbot that helps people choose what to wear today. Based on the user's mood and preferences, you need to help them choose the right outfit and print it out as an image. To make outfit recommendations, you need the following information.
#     < base conditions : don't ask >
